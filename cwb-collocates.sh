...
cat <<-END
DESCRIPTION:
    CWB wrapper around CQP tabulate and cwb-scan-corpus to create
...
USAGE:
    ...
    -c|--match) include match frequency; see CQP count
...
# if stdin, buffer file; if neither stdin nor -f, create file
if [ -p /dev/stdin ]; then
  cat /dev/stdin > "$data"
elif [ ! "${file:-}" ]; then
  query="$2"
